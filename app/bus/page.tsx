...
const RouteAnimation = ({ route, isOpen, onClose }: { route: Route; isOpen: boolean; onClose: () => void }) => {
  const stops = route.stops.split('→').map(stop => stop.trim());
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="bg-gray-900/90 p-6 rounded-xl border-2 border-violet-500/20 max-w-2xl w-full mx-4 relative"
            onClick={e => e.stopPropagation()}
          >
            <button
              onClick={onClose}
              className="absolute top-4 right-4 text-violet-400/70 hover:text-violet-400"
            >
              <FiX size={24} />
            </button>
            
            <h2 className="text-2xl font-bold text-violet-400 mb-2">
              {route.place} - #{route.busNumber}
            </h2>
                ...
                return (
                  <div key={stop} className="relative mb-6 pl-12">
                    ...
                    {isCurrentStop && (
                      ...
                      >
                        <FaBus className="text-violet-400 text-xl" />
                      </motion.div>
                    )}
                  </div>
                );
  ...
  return (
    <>
      <motion.div 
        ...
        className="relative border-2 border-violet-500/20 bg-gray-900/30 backdrop-blur-sm rounded-xl p-4 sm:p-6 mb-4 transition-all cursor-pointer"
        ...
        <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
          ...
          <div className="flex-grow">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-xs sm:text-sm text-violet-400/80">Click to view route animation</span>
            </div>
          </div>
        </div>
...
const busRoutes: Route[] = [
  {
      place: "Thiruvanmiyur",
      busNumber: "111",
      stops: "Thiruvanmiyur Bus Terminus (6:20) → Adyar Depot (6:30) → Adyar Signal (6:35) → Madhya Kailash (6:37) → Guindy Race Course (6:50) → Nehru Statue (6:52) → Kasi Theatre (6:55) → Ashok Pillar (7:00) → Vadapalani Signal (7:10) → MMDA (7:12) → Koyambedu Roundana (7:15) → College (8:10)"
    },
    {
      place: "Porur",
      busNumber: "121",
  ...
  },
  {
    place: "T.Nagar",
      busNumber: "141",
      stops: "T.Nagar Panagal Park (6:45) → Pondy Bazaar (6:50) → Vani Mahal (6:52) → Vidhyodaya School → Valluvarkottam (6:57) → Nungambakkam Police Station → Sterling Road → Choolaimedu (7:00) → Arun Hotel (7:05) → Anna Arch → Roundtana (7:10) → K4 → Nathamuni (7:20) → Senthil Nagar (7:25) → College (8:00)"
    },
    {
      place: "Kodambakkam",
      busNumber: "145",
    ...
    },
    {
      place: "Uthukottai",
      busNumber: "195",
      stops: "Uthukottai (7:10) → Palavakkam (7:25) → Periyapalayam (7:40) → Arani (7:50) → College (8:10)"
    },
    {
      place: "Tambaram",
      busNumber: "112",
    ...
    },
    {
      place: "Kasimedu",
      busNumber: "173",
      stops: "Kasimedu R2 PS (6:50) → Kalmandapam → Royapuram Market (6:55) → Stanley Hospital → Mint (7:05) → Basin Bridge → M.K.B Nagar → M.R. Nagar → Erukencherry → Moolakadai (7:15) → Madhavaram Roundana → 100 ft Road (7:20) → College (8:00)"
    },
    {
      place: "Manali Market",
      busNumber: "175",
      stops: "Manali Market (7:00) → Mathur (7:05) → Milk Colony FG → SG (7:10) → Arul Nagar (7:15) → Moolakadai (7:25) → College (8:00)"
    },
    {
      place: "Tiruvottiyur",
      busNumber: "181",
      stops: "Rajakadai (6:35) → Kaladipet → Thiruvottiyur Police Station → Ellaiamman Koil (6:40) → Theradi → Vellanchetti School → Ajax (6:50) → Periyar Nagar → Wimco → Ennore Lift Gate (6:55) → Jothi Nagar (6:57) → Sathyamoorthy Nagar → MFL Corner → College (8:00)"
    },
    {
      ...
      <div className="max-w-4xl mx-auto min-h-full">
        <div className="relative mb-16">
          ...
          <motion.div
            ...
            className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 w-24 h-1 bg-violet-500/20 rounded-full"
          />
        </div>
